...
if [ ! -f "$NAME"".avi" ]; then
  ...
  ffmpeg -f avfoundation -video_size 1280x720 -framerate "$FPS" -i "Micro:none" -crf 28 -vcodec libx264 -y -t "$LENGTH" "$NAME"".avi"
  ...
  printf "a video file with that name already exists."
fi
