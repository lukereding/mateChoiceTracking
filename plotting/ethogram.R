...
draw_rectangle <- function(start_x,end_x,y,offset,color,chunk_length){
  ...
  rect(start_x,y+offset,end_x,y-offset,col=color, border = NA )
  
}


draw_rectangles <- function(sides,i=1,end_i = 100,y_position,size,offset,part=1){
  i_initial = i
  ...
  while(i < end_i){
    ...
    if(new_i %% size >= i %% size){
      draw_rectangle(i - i_initial,new_i - i_initial,y_position,color = c("#482878FF","white","#35B779FF")[match(sides[i],levels(as.factor(sides)))],chunk_length=size,offset=offset)
      ...
      draw_rectangle(i - i_initial,end_i - i_initial,y_position,color = c("#482878FF","white","#35B779FF")[match(sides[i],levels(as.factor(sides)) %>% sort)],chunk_length=size,offset=offset)
      i <- end_i
    }
  }
}
...
ethogram <- function(path,name){
  ...
  draw_rectangles(sides_vector,i=1,end_i = bounds[2], y_position=bounds[2],size=bounds[2],part=1,offset=off)
  print("drew part 1")
  draw_rectangles(sides_vector,i=bounds[3],end_i = bounds[4], y_position=bounds[2]*0.75,size=bounds[2],part=2,offset=off)
  print("drew part 2")
  draw_rectangles(sides_vector,i=bounds[5],end_i = bounds[6], y_position=bounds[2]*0.5,size=bounds[2],part=3,offset=off)
  print("drew part 3")
  draw_rectangles(sides_vector,i=bounds[7],end_i = bounds[8], y_position=bounds[2]*0.25,size=bounds[2],part=4,offset=off)
  print("drew part 4")
  draw_rectangles(sides_vector,i=bounds[9],end_i = bounds[10], y_position=bounds[2]*0,size=bounds[2],part=5,offset=off)
  print("drew part 5")
  # draw legend
  legend(x = bounds[2]*0.45, y= bounds[2]*0.55, c("left","right","neutral"), fill=c("#482878FF","#35B779FF","white"),border="black",ncol = 3,bty="o", box.lwd=0.3) 
